...
from bursar.forms import PaymentMethodForm, CustomChargeForm
from bursar.views import contact
...
def balance_remaining(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not modulename.startswith('GATEWAY_'):
                modulename = 'GATEWAY_' + modulename
    ...
    ctx = RequestContext(request, {'form' : form, 
        'order' : order,
        'paymentmethod_ct': len(config_value('GATEWAY', 'MODULES'))
    })
