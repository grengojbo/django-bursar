...
def cron_rebill(request=None):
    ...
    if request is not None:
        if not config_value('GATEWAY', 'ALLOW_URL_REBILL'):
            return bad_or_missing(request, _("Feature is not enabled."))
        if 'key' not in request.GET or request.GET['key'] != config_value('GATEWAY','CRON_KEY'):
...
#                        item.order.save()
                    ...
                    #list of ipn based payment gateways.  Include processors that use 3rd party recurring billing.
                    ipn_based = ['PAYPAL']
                    if not payments.payment in ipn_based and item.order.balance > 0:
                        ...
                        payment_module = config_get_group('GATEWAY_%s' % payments.payment)
