...
class TestRecurringBilling(TestCase):
    ...
    def testCronRebill(self):
        ...
        from django.conf import settings
        self.response = self.c.get(url, data={'key': config_value('GATEWAY','CRON_KEY')})
        self.assertEqual(self.response.status_code, 200)
...
class TestModulesSettings(TestCase):
    ...
    def setUp(self):
        self.dummy = config_get_group('GATEWAY_DUMMY')
...
#     def setUp(self):
#         self.charset = config_value('GATEWAY_GATEWAY_GIFTCERTIFICATE', 'CHARSET')
#         self.format = config_value('GATEWAY_GATEWAY_GIFTCERTIFICATE', 'FORMAT')
# 
#     def testGetCode(self):
...
class TestMinimumOrder(TestCase):
    ...
    def test_checkout_minimums(self):
        ...
        """
        min_order = config_get('GATEWAY', 'MINIMUM_ORDER')
